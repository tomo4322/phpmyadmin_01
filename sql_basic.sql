...
SELECT celebrities.name, countries.name, countrylanguages.language FROM celebrities JOIN countries ON countries.code = celebrities.country_code JOIN countrylanguages ON countries.code = countrylanguages.country_code WHERE countrylanguages.is_official = 'T';


-- 問29
-- 全ての有名人の名前と国名をテーブル結合せずに出力してください。

SELECT celebrities.name, countries.name FROM celebrities, countries WHERE countries.code = celebrities.country_code;


-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。

SELECT country_code, MAX(age) AS ' MAX(ce.age)', MIN(age) AS 'MIN(ce.age)' FROM celebrities GROUP BY country_code HAVING MAX(age) > 50 AND MIN(age) < 30 AND country_code != ' ';


-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。

SELECT '1980' AS '誕生年' , COUNT(name) FROM celebrities WHERE birth BETWEEN '1980-1-1' AND '1980-12-31' UNION SELECT '1981', COUNT(name) FROM celebrities WHERE birth BETWEEN '1981-1-1' AND '1981-12-31';
